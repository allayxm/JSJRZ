...
namespace MXKJ.BusinessLogic
{
    public class Student : BusinessLogicBase
    {
        ...
        public int GetGradeIDByClass(int ClassID)
        {
            int vResult = 0;
            DataTable vTable = m_BasicDBClass.SelectCustom(string.Format("Select *From edu_org where ID={0}", ClassID));
            if (vTable.Rows.Count > 0)
                vResult = DBConvert.ToInt32(vTable.Rows[0]["parent_id"]).Value;
            return vResult;
        }

        public StudentStruct[] QueryStudent_HouseAllot(int GradeID, int ClassID, string StudentName)
        {
